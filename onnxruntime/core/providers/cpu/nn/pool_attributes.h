...
struct PoolAttributes {
  ...
  PoolAttributes(const OpNodeProtoHelper<ProtoHelperNodeContext>& info,
    ...
    ORT_ENFORCE(info.GetAttrShape("kernel_shape", kernel_shape).IsOK(),
    ...
    if (!info.GetAttrShape("pads", pads).IsOK() || pads.empty()) {
      pads.resize(kernel_shape.size() * 2, 0);
    }

    if (!info.GetAttrShape("strides", strides).IsOK() || strides.empty()) {
    ...
    if (!info.GetAttrShape("dilations", dilations).IsOK() || dilations.empty()) {
